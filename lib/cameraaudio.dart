...
class _VideoQuestionnairePageState extends State<VideoQuestionnairePage> {
  ...
  Future _stopRecording() async {
    ...
    final sanitizedQuestion = rawQuestion.replaceAll(' ', '_').replaceAll(RegExp(r'[^\w\-]'), '');
  ...
  // --- CHANGES START HERE ---
  Future _uploadImagesToSupabase(
    String folderName,
    List<File> files,
  ) async {
    final supabase = Supabase.instance.client;
    for (var file in files) {
      ...
      try {
        ...
        debugPrint('Upload failed for $fileName: $e');
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Upload failed for $fileName")));
      }
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Images uploaded successfully.")),
